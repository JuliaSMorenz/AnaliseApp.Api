...
namespace AnaliseApp.Infra.Data
{
    public class Repository
    {
        ...
        public List<Financeiro> Get(DateTime dataInicio, DateTime dataFim, Guid PessoaId)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                string query = @"SELECT * 
                                FROM Financeiro 
                                WHERE DataEmissao >= @DataInicio AND DataEmissao <= @DataFim AND PessoaId = @PessoaId
